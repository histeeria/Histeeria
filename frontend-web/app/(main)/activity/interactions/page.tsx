...
import { Loader2, Heart, MessageCircle, ArrowLeft, AlertCircle, RefreshCw } from 'lucide-react';
...
function InteractionsPageContent() {
  ...
  const {
    ...
    queryKey: ['activity', 'interactions', filter] as const,
    queryFn: async ({ pageParam = 0 }: { pageParam: number }) => {
      const response = await postsAPI.getUserInteractions(filter, pageParam, 20);
      if (!response.success) {
  ...
  const posts = interactionsData?.pages.flatMap((page: any) => page.posts || []) || [];
